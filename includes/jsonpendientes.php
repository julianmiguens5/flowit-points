...
$sql = "SELECT *, DATE_FORMAT(fechacanje,'%d-%m-%Y') AS fechala FROM `vouchers` WHERE `canjeado`= 0 ";
$query=mysqli_query($conn, $sql) or die("jsonhistorial.php: get historial");
$totalData = mysqli_num_rows($query);
$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.


$sql = "SELECT *, DATE_FORMAT(fechacanje,'%d-%m-%Y') AS fechala FROM `vouchers` WHERE 1=1 AND `canjeado`= 0 ";
